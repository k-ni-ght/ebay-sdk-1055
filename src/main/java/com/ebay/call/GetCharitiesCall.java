...
import com.ebay.soap.eBLBaseComponents.GetCharitiesResponseType;
 ...
 * <br> <B>Input property:</B> <code>CharityName</code> - This field will accept the full or partial name of a registered nonprofit organization. For example, if you pass in a value of <b>heart</b>, all registered nonprofit organizations with "heart" in their name may be returned in the response. If the <b>MatchType</b> field is used and set to <code>StartsWith</code>, the phrase "heart" must come at the beginning of the nonprofit organization's name. However, if the <b>MatchType</b> field is used and set to <code>Contains</code>, or is not used at all, all registered nonprofit organizations with "heart" in their name should be returned in the response. The string passed in this field is not case-sensitive.
 * <br/><br/>
 ...
 * <br> <B>Input property:</B> <code>CharityDomain</code> - This field can be used to search for registered nonprofit organizations within a specified charity domain or category. Each charity domain will have an integer value associated with it. This integer value is what is passed in to this field. Nonprofit organizations can be associated with multiple charity domains.
 * <br> <B>Input property:</B> <code>IncludeDescription</code> - This field should be included and set to <code>true</code> if the <b>Query</b> field is used, and the user wishes to search the charity's name and its mission statement. If this field is omitted or included and set to <code>false</code>, the call will only search charity names for the specified string in the  <b>Query</b> field.
