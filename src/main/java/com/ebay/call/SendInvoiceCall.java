...
import com.ebay.soap.eBLBaseComponents.SendInvoiceResponseType;
 ...
 * <br> <B>Input property:</B> <code>InsuranceOption</code> - This field is no longer applicable as it is not longer possible for a seller to offer a buyer shipping insurance.
 * <br> <B>Input property:</B> <code>InsuranceFee</code> - This field is no longer applicable as it is not longer possible for a seller to offer a buyer shipping insurance.
 * <br> <B>Input property:</B> <code>PaymentMethods</code> - This optional field allows a US or German seller to add specific payment methods that were not in the original item listing. The only valid values for this field are 'PayPal' for a US listing, or 'MoneyXferAcceptedInCheckout' (CIP+) for a DE listing.
 ...
 * <br> <B>Input property:</B> <code>EmailCopyToSeller</code> - Flag indicating whether or not the seller wishes to receive an email copy of the invoice sent to the buyer.
 ...
 * request.
 * <br> <B>Input property:</B> <code>AdjustmentAmount</code> - This field allows the seller to adjust the total cost of the order to account for an extra charge or to pass down a discount to the buyer. <br><br> The currency used in this field must be the same currency of the listing site. A positive value in this field indicates that the amount is an extra charge being paid to the seller by the buyer, and a negative value indicates that the amount is a discount given to the buyer by the seller.
 *
 * @author Ron Murphy
 * @version 1.0
 */
