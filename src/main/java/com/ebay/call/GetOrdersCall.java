 ...
 * <br> <B>Input property:</B> <code>OrderIDArray</code> - A container for eBay order IDs. If one or more order IDs are specified in this container, no other call-specific input fields are applicable.
 ...
 * <br> <B>Input property:</B> <code>OrderRole</code> - Filters the returned orders based on the role of the user. The user's role is either buyer or seller. If this field is used with a date filter, returned orders must satisfy both the date range and the <b>OrderRole</b> value.
 * <br> <B>Input property:</B> <code>OrderStatus</code> - <span class="tablenote"><b>Note: </b> Since the Half.com site has been shut down, the <code>Shipped</code> value is no longer applicable since this value is only used for retrieving Half.com listings.
 * </span>
 * The field is used to retrieve eBay orders that are in a specific state. If this field is used with a date filter, only orders that satisfy both the date range and the <b>OrderStatus</b> value are retrieved. <br><br> If one or more <b>OrderID</b> values are specified through the <b>OrderIDArray</b> container, the <b>OrderStatus</b> field should not be used, and it is ignored if it is used.
 ...
 * <br>
 * <br> <B>Input property:</B> <code>IncludeFinalValueFee</code> - Indicates whether to include the Final Value Fee (FVF) for all Transaction objects in the response. The Final Value Fee is returned in Transaction.FinalValueFee. The Final Value Fee is assessed right after the creation of an eBay order line item.
 * <br>
